// ============================================================
// SIDEBAR COMPONENT — compartilhado entre todas as páginas
// ============================================================
export function getSidebarHTML(activePage = '') {
  const pages = [
    ...
    { id: 'clientes',      href: 'clientes.html',      icon: 'fa-users',         label: 'Clientes',        section: 'Principal' },
    { id: 'servicos',      href: 'servicos.html',       icon: 'fa-tags',          label: 'Serviços',        section: 'Gestão' },
    { id: 'equipe',        href: 'equipe.html',         icon: 'fa-user-tie',      label: 'Equipe',          section: 'Gestão' },
    { id: 'financeiro',    href: 'financeiro.html',     icon: 'fa-dollar-sign',   label: 'Financeiro',      section: 'Gestão' },
  ...
  Object.entries(sections).forEach(([section, items]) => {
    ...
    items.forEach(item => {
      ...
      html += `
      <a href="${item.href}" class="nav-item${active}"${adminAttr} data-page="${item.id}">
  ...
  <div id="sessionTimerBanner" class="session-banner">
    <i class="fas fa-clock"></i> <span id="sessionTimerText">Sessão expirando…</span>
    <button onclick="document.getElementById('sessionTimerBanner').classList.remove('show')" style="margin-left:12px;background:none;border:none;cursor:pointer;color:#000;font-weight:700">×</button>
  </div>`;
